...
#SBATCH --output /home/ahmadkhana/Desktop/etiv-comparison/icvmapp3r/log/slurm_output/%A_%a.out # saves output as (jobID)_out.out 
#SBATCH --error /home/ahmadkhana/Desktop/etiv-comparison/icvmapp3r/log/slurm_output/%A_%a.err # saves error as (jobID)_err.out 
#SBATCH --ntasks=8
#SBATCH --cpus-per-task 8
#SBATCH --time 6:00:00 # change to the time you think your job will need. There is no upper limit. 
#SBATCH --mem 50000 # the amount of memory (here in MB) your job requires. If you pick too little your job will get stuck and die. 
...
BASE="/home/ahmadkhana/Desktop/etiv-comparison/icvmapp3r"
IMG="$BASE/singularity/icvmapper.sif"
...
for subj_i in $(seq 0 $(SUBJECTS_PER_JOB - 1)); do
    ...
    subj=$(basename "$(dirname "$nii_file")")
    dataset=$(basename "$(dirname "$(dirname "$nii_file")")")
