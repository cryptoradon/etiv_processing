...
parser.add_argument("--results_dir", type=str, default="/red/nvidia-ai/SkylarStolte/training_pairs_v5_bfc/", help="directory the results should be in")
...
checkpoint_path = os.path.join(data_dir, args.model_load_name)
pretrained_dict = torch.load(checkpoint_path, map_location=device)


# Adjust the keys by removing the 'module.' prefix if present
pretrained_dict = {key.replace("module.", ""): value for key, value in pretrained_dict.items()}

# Load the adjusted state_dict into the m~/Desktop/etiv-comparison/grace/data/bert_test.matodel
missing_keys, unexpected_keys = model.load_state_dict(pretrained_dict)
...
for i in range(case_num):
    ...
    savepath = os.path.join(results_dir, filename + '.mat')
    savemat(savepath, {'testimage':testimage})#, 'logits':logits})
    #print("--- %s seconds ---" % (time.time() - start_time))

    # Save in correct format
    import nibabel as nib
    testimage = torch.argmax(test_outputs, dim=1).detach().cpu().numpy()[0]
    affine = test_ds[i]["image_meta_dict"]["affine"]
    nii_out = nib.Nifti1Image(testimage.astype(np.uint8), affine)
    converted_result_dir = os.path.join(results_dir, "output_converted")
    os.makedirs(converted_result_dir, exist_ok=True) 
    nib.save(nii_out, os.path.join(converted_result_dir, f"{filename}.nii.gz"))
